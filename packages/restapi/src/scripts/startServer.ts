...
async function startServer(): Promise<void> {
  ...
  const fastifyAdapter: FastifyAdapter = new FastifyAdapter(fastifyServerOptions);

  const nestApplicationOptions: NestApplicationOptions = {
    cors: { origin: '*' },
  };
  const nestApplication: NestFastifyApplication = await NestFactory.create<NestFastifyApplication>(
    AppModule,
    fastifyAdapter,
